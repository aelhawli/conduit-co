...
The replacement has now been verified and installed as encrypted `GEMINI_API_KEY` on the staging Worker. The user created it in another isolated project: actual ID `gen-lang-client-0675067246`, display name `gen-lang-client-0948341518`. Key name `conduit-staging-gemini-v2`; the exposed predecessor is no longer listed. The replacement was transferred in browser memory without emitting its value, then temporary state and clipboard were cleared.

Hosted testing returned Gemini HTTP 404 for `gemini-2.5-flash`. Google documents that new projects cannot access the legacy 2.5 models (https://ai.google.dev/gemini-api/docs/deprecations). Staging only now uses `gemini-3.8-flash`. Added numeric-status-only upstream diagnostics, with a test proving private provider bodies are absent from logs and client responses. This fixes diagnosis without logging provider bodies, headers, secrets, or PDFs. Production model configuration is unchanged and must be reviewed before a future release.

The real desktop flow subsequently passed from PDF upload to Gemini preview, saved lead and unlocked risks. Supabase confirms job `ebb471fb-abb6-4997-b447-dfb74d198d9c`, lead `ff62c1c1-ad43-40a0-a6bd-a6fefdea0614`, consent, attribution, and all eight funnel event types. Tests now total 75 automated plus 10 mocked browser tests, all passing. Latest hosted HTTP checks: 20 passing; dependency audit: zero known vulnerabilities. Mobile and valid repeat-upload completion are still in progress.
