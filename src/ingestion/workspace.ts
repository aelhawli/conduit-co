...
async function refreshProject(){
 ...
 $('documents').replaceChildren(...p.documents.map(v=>{const a=document.createElement('article'),h=document.createElement('h3'),status=document.createElement('p');h.textContent=`${v.original_filename} · Version ${v.version}`;
  const labels:Record<string,string>={UPLOADING:'Waiting for upload completion. Select the same file to resume.',UPLOADED:'Upload received. Waiting for validation.',VALIDATING:'Conduit is validating your PDF.',PROCESSING:`PDF validated. Preparing pages: ${v.job?.pages_prepared??0} of ${v.page_count??'unknown'}.`,COMPLETED:`Ready: ${v.page_count} pages validated and prepared.`,FAILED:`Document could not be prepared: ${v.job?.error_code??'processing failed'}.`,CANCELLED:'Cancelled. Cleanup is pending.'};status.textContent=labels[v.state]??v.state;
...
async function upload(file:File){
 ...
 let version=selectedVersions.find(v=>v.state==='UPLOADING'&&(v as Version&{original_filename:string}).original_filename===file.name&&v.declared_bytes===file.size);
 if(version){version=await api<Version>('resume',{project_id:projectId,version_id:version.id});if((version as Version&{fingerprint:string}).fingerprint!==fp)throw new Error('The selected file differs from the saved upload. Choose the original file.');}
  ...
  const key=`conduit-upload:${projectId}:${fp}`;const idempotency_key=localStorage.getItem(key)??crypto.randomUUID();localStorage.setItem(key,idempotency_key);
  try{version=await api<Version>('reserve',{project_id:projectId,filename:file.name,media_type:'application/pdf',byte_size:file.size,fingerprint:fp,idempotency_key,turnstileToken:challenge});}finally{challenge='';const turnstile=(window as unknown as {turnstile?:Turnstile}).turnstile;if(widget)turnstile?.reset(widget);}
 }
 ...
 await api('complete',{project_id:projectId,version_id:v.id});localStorage.removeItem(`conduit-upload:${projectId}:${fp}`);message('Upload received. Processing continues if you close this page.');await refreshProject();
}
...
$<HTMLFormElement>('create').onsubmit=async e=>{e.preventDefault();try{const p=await api<{id:string}>('create_project',{id:crypto.randomUUID(),organisation_id:$<HTMLSelectElement>('organisation').value,name:$<HTMLInputElement>('project-name').value});await loadProjects();await openProject(p.id);}catch(e){message((e as Error).message);}};
...
void start().catch(e=>message((e as Error).message));setInterval(()=>{if(projectId&&!document.hidden)void refreshProject().catch(()=>message('Connection interrupted. Your tender is saved; refresh when connected.'));},5000);
