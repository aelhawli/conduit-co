...
export class IngestionDatabase{
 ...
 async authenticate(token:string){
  const r=await this.fetcher.call(globalThis,`${STAGING_DB}/auth/v1/user`,{headers:{apikey:this.env.SUPABASE_PUBLISHABLE_KEY,Authorization:`Bearer ${token}`},signal:AbortSignal.timeout(10000)});
